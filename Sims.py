...
class FedAILM_Client_Sim:
    def __init__(self,rand_percent,layerIndex,cid, Loader, Model, Lr, wdecay, epoch=1, fixlr=False, optzer="SGD",Depochs=False,topk=20):
        ...
        self.eta = 1.0
        self.AILM = AILM(cid,self.loss, self.TrainData, self.batch_size, 
    ...
    def local_initialization(self,SCPR,received_global_model):
        self.AILM.adaptive_local_aggregation(received_global_model, self.Model)
...
class FedAILM_SCPR_Client_Sim:
    ...
    # 全局模型参数与局部模型进行聚合
    def local_initialization(self,serverCpr, received_global_model):
        if serverCpr==True:#如果使用服务器压缩，那么对应的客户端合并也应该改变
            self.AILM.global_local_aggregation(received_global_model, self.Model)
        else:
            self.AILM.adaptive_local_aggregation(received_global_model, self.Model)
